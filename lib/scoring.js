...
// Between two consecutive brackets -> continuous linear interpolation (confirmed against
// worked examples the user provided, e.g. avg concurrent 15 -> 61, fans 1500 -> 62, hours 1000 -> 62.4-equivalent).
export function interpScore(value, ascTable) {
  const v = value || 0;
  const n = ascTable.length;
  if (v < ascTable[0][0]) return 50;
  if (v >= ascTable[n - 1][0]) return ascTable[n - 1][1];
  for (let i = 0; i < n - 1; i++) {
    const [aTh, aSc] = ascTable[i];
    const [bTh, bSc] = ascTable[i + 1];
    if (v >= aTh && v < bTh) {
      const t = (v - aTh) / (bTh - aTh);
      return aSc + (bSc - aSc) * t;
    }
  }
  return ascTable[n - 1][1];
}
